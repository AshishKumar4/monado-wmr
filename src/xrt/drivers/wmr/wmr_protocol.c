...
/* The absolute accel scale (LSB -> m/s^2) is per-DEVICE calibration, owned by the caller
 * (wmr_hmd: per-headset-type default + config-dir refinement) -- never a protocol constant. */
void
vec3_from_hololens_accel(int32_t sample[3][4], int i, float scale, struct xrt_vec3 *out_vec)
{
	out_vec->x = (float)sample[0][i] * scale;
	out_vec->y = (float)sample[1][i] * scale;
	out_vec->z = (float)sample[2][i] * scale;
}
